...
<% 
    ...
    String id="";   // 학번 초기화
    try{    // 다른 페이지로 이동 시
        id=request.getParameter("id");   // 학번 인자로 받음 
    ...
    try{
        ...
        if(first){  // 처음 시작이면
            ...
            PreparedStatement pstmt = conn.prepareStatement("UPDATE examtable SET name=?, kor=?, eng=?, mat=? WHERE id = ?");   // 수정하는 sql문
            pstmt.setString(1, name);   // 이름
            pstmt.setInt(2, Integer.parseInt(kor)); // 국어 성적
            pstmt.setInt(3, Integer.parseInt(eng)); // 영어 성적
            pstmt.setInt(4, Integer.parseInt(mat)); // 수학 성적
            pstmt.setInt(5, Integer.parseInt(id));  // 학번
            pstmt.executeUpdate();  // sql문 실행
            
            pstmt = conn.prepareStatement("select count(*) from examtable where id<"+id+";");   // 해당 학생의 학번 위치
            ResultSet rset2 = pstmt.executeQuery();  // sql문 실행
      
            while(rset2.next()){    // 해당 값 가져오기
                fromPT=rset2.getInt(1)/cntPT*cntPT; // fromPT구하기
            }
        ...
        "(select count(*)+1 from examtable as m where m.kor+m.eng+m.mat > n.kor+n.eng+n.mat) as ranking from examtable as n order by id;"); // 모든 학생 정보+총점,평균,등수 가져오기 	
%>
<h1> 레코드 수정</h1>   <%--제목--%>
...
%>
    <tr>    <%--다음 행--%>
    <%--수정한 정보의 배경색은 노란색으로 변경--%>
    <td <%if(studentid.equals(id)) out.println("bgcolor=yellow");%>><a href="./oneviewDB.jsp?studentname=<%=studentname%>&studentid=<%=studentid%>"><%=studentname%></a></td>  <%--이름,하이퍼링크--%>
    <td <%if(studentid.equals(id)) out.println("bgcolor=yellow");%>><%=studentid%></td>  <%--학번--%>
    <td <%if(studentid.equals(id)) out.println("bgcolor=yellow");%>><%=Integer.toString(rset.getInt(3))%></td>  <%--국어--%>
    <td <%if(studentid.equals(id)) out.println("bgcolor=yellow");%>><%=Integer.toString(rset.getInt(4))%></td>  <%--영어--%>
    <td <%if(studentid.equals(id)) out.println("bgcolor=yellow");%>><%=Integer.toString(rset.getInt(5))%></td>  <%--수학--%>
    <td <%if(studentid.equals(id)) out.println("bgcolor=yellow");%>><%=Integer.toString(rset.getInt(6))%></td>  <%--총점--%>
    <td <%if(studentid.equals(id)) out.println("bgcolor=yellow");%>><%=Integer.toString(rset.getInt(7))%></td>  <%--평균--%>
    <td <%if(studentid.equals(id)) out.println("bgcolor=yellow");%>><%=Integer.toString(rset.getInt(8))%></td>  <%--등수--%>
    </tr>
<% 
        }   // 반복문 종료
...
%>
    ...
    %>
    <a href="updateDB.jsp?from=0&cnt=<%=cntPT%>&id=<%=id%>">&lt;&lt;</a>   <%--첫페이지(<<)--%>
    <%
        }   // (<<) 출력 조건 종료
    %>
    <%
        if(current>block){  // 현재페이지가 맨앞 페이지가 아닌 경우
    %>
    <a href="updateDB.jsp?from=<%=left%>&cnt=<%=cntPT%>&id=<%=id%>">&lt;</a>   <%--이전(<)--%>
    ...
    %>    
        <a href="updateDB.jsp?from=<%=pagefrom%>&cnt=<%=cntPT%>&id=<%=id%>" style="color:red; font-weight: bold;"><%=pagenum%></a>   <%--페이지 번호와 색과 강조 변경--%>
    <%
            } else{
    %>
        <a href="updateDB.jsp?from=<%=pagefrom%>&cnt=<%=cntPT%>&id=<%=id%>"><%=pagenum%></a>   <%--페이지 번호와 그에 해당하는 값들을 인자로 하이퍼링크--%>
    <%  
            } // 조건문 종료
        }   // 페이지 출력 반복문 종료
        if(maxstartpage>current){    // 맨뒤 페이지인 경우
    %>
    <a href="AllviewDB.jsp?from=<%=right%>&cnt=<%=cntPT%>&id=<%=id%>">&gt;</a>   <%--다음(>)--%>
    <%  }   // (>) 출력 조건 종료
        if(maxstartpage>current){    // 맨뒤 페이지인 경우
    %>
    <a href="updateDB.jsp?from=<%=lastfrom%>&cnt=<%=cntPT%>&id=<%=id%>">&gt;&gt;</a>   <%--마지막페이지(>>)--%>
    <%  } %>  <%--(>>) 출력 조건 종료--%>
