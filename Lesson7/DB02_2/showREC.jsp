...
%>
    ...
    <form method="post" action="showREC.jsp">    <!--입력한 내용 보낼 곳-->
        <div>   <!--안쪽 공간 지정-->
        조회할 학번 <input type="number" name="id">   <!--조회할 학번 입력하는 창-->
    ...
    <form method="post">    <!--폼형식:post-->
        <table> <!--테이블 생성-->
            <tr>    <!--1행-->
                <td>이름</td>   <!--1열-->                    
                <td>    <!--2열--> 
                    <input style="text-align: center;" type="text" id="name" name="name" maxlength='20' value="<%=stuname%>" onkeyup="checkName(event)" <%if(stuid.equals("")){out.println("readonly");}%>>    
                                                                                                                                                                    <!--이름 입력 필드, 확인 이벤트 추가-->
                    <br>    <!--다음 줄-->
            ...
            <tr>    <!--3행-->
                <td>국어</td>   <!--1열-->                    
                <td>    <!--2열--> 
                    <input style="text-align: center;" type="number" id="kor"  name="korean" value="<%=korean%>" onkeyup="checkInput(event)" <%if(stuid.equals("")){out.println("readonly");}%>>    <!--국어 입력 필드, 확인 이벤트 추가-->
                    <br>    <!--다음 줄-->
            ...
            <tr>    <!--4행-->
                <td>영어</td>   <!--1열-->                 
                <td>    <!--2열--> 
                    <input style="text-align: center;" type="number" id="eng" name="english" value="<%=english%>" onkeyup="checkInput(event)" <%if(stuid.equals("")){out.println("readonly");}%>>   <!--영어 입력 필드, 확인 이벤트 추가-->
                    <br>    <!--다음 줄-->
            ...
            <tr>    <!--5행-->
                <td>수학</td>   <!--1열-->                 
                <td>    <!--2열-->    
                    <input style="text-align: center;" type="number" id="mat" name="math" value="<%=math%>" onkeyup="checkInput(event)" <%if(stuid.equals("")){out.println("readonly");}%>>  <!--수학 입력 필드, 확인 이벤트 추가-->
                    <br>    <!--다음 줄-->
...
<script>  
    ...
    function updateSubmitButtonState(e) {    // 버튼 활성 여부 함수
        ...
        if(errors.name||errors.kor||errors.eng||errors.mat){ // 에러가 있는 경우            
            if(errors.name){    // 이름 입력 시 에러이면
                checkName();    // 이름 에러 메시지 출력
            }
            if(errors.kor){ // 국어 입력 시 에러이면
                checkScore({ target: kor });    // 국어 에러 메시지 출력
            }
            if(errors.eng){ // 영어 입력 시 에러이면
                checkScore({ target: eng });    // 영어 에러 메시지 출력
            }
            if(errors.mat){ // 수학 입력 시 에러이면
                checkScore({ target: mat });    // 수학 에러 메시지 출력
            }
