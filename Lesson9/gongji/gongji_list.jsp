...
<body>	<!--본문-->
	...
	try{
		...
		ResultSet rset = stmt.executeQuery("select count(*) from gongji;"); // 모든 공지를 가져오는 sql문
        if(rset.next()){            
            ...
            rset = stmt.executeQuery("select * from gongji order by id desc;"); // 모든 공지를 가져오는 sql문
...
<div style="width:900px">   <!--바깥쪽-->
    ...
    <table cellspacing=3 border=1> <!--공지 리스트-->    
        <tr>    <!--1행-->
            ...
            <th style="text-align : center;width:100px;">등록일</th> <!--3열-->
        </tr>   <!--1행 끝-->
    <%  
            while(rset.next()){ // 가져온 모든 정보들에 대하여
                ...
                String date=rset.getString(3);  // 날짜
    ...
    %>
        ...
        <tr>    <!--행 시작-->       
            ...
            <td width="700px"><a href="./gongji_view.jsp?key=<%=key%>"><%=title%></td>  <!--2열, 제목--> 
            <td style="text-align : center;width:100px;"><%=date%></td> <!--3열, 등록일-->
        </tr>   <!--행 종료-->
        </form> <!--폼 종료-->
    <%
        }   // 반복문 종료           
